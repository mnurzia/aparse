...
MN_API void aparse_set_out_cb(aparse_state *state, aparse_out_cb out_cb,
                              void *user) {
  aparse__state_set_out_cb(state->state, out_cb, user);
}

MN_API aparse_error aparse_add_opt(aparse_state *state, char short_opt,
                                   const char *long_opt) {
...
MN_API void aparse_arg_type_str(aparse_state *state, const char **out,
                                mn_size *out_size) {
  aparse__state_check_before_set_type(state->state);
...
MN_API void aparse_arg_type_custom(aparse_state *state, aparse_custom_cb cb,
                                   void *user, aparse_nargs nargs) {
  aparse__state_check_before_set_type(state->state);
  aparse__arg_custom_init(state->state->tail, cb, user, nargs);
}

MN_API aparse_error aparse_parse(aparse_state *state, int argc,
